...
def verify_result(pred_extract, ori_info):
    ...
    debug_eval = str(os.environ.get("AMO_BENCH_DEBUG_EVAL", "")).strip().lower() in ("1", "true", "yes", "on")
    if debug_eval:
        print(
            f"[AMO ROUTE] question_id={info.get('question_id')} "
            f"answer_type={info.get('answer_type')}"
        )

    if info["answer_type"] == "description":
        if debug_eval:
            print("[AMO ROUTE] -> verify_description_answer (LLM)")
        result = verify_description_answer(pred_extract, info)
        
    elif info["answer_type"] in ["number", "set"]:
        if debug_eval:
            print("[AMO ROUTE] -> verify_number_set_answer (parser/symbolic)")
        result = verify_number_set_answer(pred_extract, info)

    else:
        assert info["answer_type"] == "variable"
        if debug_eval:
            print("[AMO ROUTE] -> verify_variable_answer (symbolic)")
